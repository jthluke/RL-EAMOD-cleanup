...
if args.toy:
    ...
    # model.load_checkpoint(path=f'saved_files/ckpt/{problem_folder}/a2c_gnn_15000.pth')
    tf = env.tf
else:
    problem_folder = 'SF_5_clustered'
    training_environments = []
    file_path = os.path.join('data', problem_folder,  'SF_5_zeros.json')
    experiment = 'training_' + file_path + '_' + str(args.max_episodes) + '_episodes_T_' + str(args.T)
    energy_dist_path = os.path.join('data', problem_folder, 'energy_distance_zeros.npy')
    scenario = create_scenario(file_path, energy_dist_path)
    env = AMoD(scenario)
    # Initialize A2C-GNN
    ...
    scale_price = 0.1
    model = A2C(env=env, T=T, lr_a=lr_a, lr_c=lr_c, grad_norm_clip_a=grad_norm_clip_a, grad_norm_clip_c=grad_norm_clip_c, seed=seed, scale_factor=scale_factor, scale_price=scale_price).to(device)
    model.load_checkpoint(path=f'saved_files/ckpt/{problem_folder}/a2c_gnn_10000.pth')
    tf = env.tf
...
if test:
    wandb.log({"AVG Reward ": rewards_np.mean(), "AVG Satisfied Demand ": served_demands_np.mean(), "AVG Rebalancing Cost": episode_rebalancing_cost.mean()})
