...
def create_scenario(json_file_path, energy_file_path, seed=10):
    ...
    scenario = Scenario(spatial_nodes=spatial_nodes, charging_stations=chargers, cars_per_station_capacity=cars_per_station_capacity,  number_charge_levels=number_charge_levels, charge_time=charge_time_per_level, 
...
file_path = os.path.join('data', 'scenario_train1x1.json')
...
while(not done):
    ...
    rebAction = RebalFlowSolver(env=env, desiredAcc=desiredAcc, gurobi_env=gurobi_env).optimize()
