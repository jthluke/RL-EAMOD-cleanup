...
class AMoD:
    ...
    def pax_step(self, paxAction=None, pax_flows_solver=None):
       ...
       for k in range(len(self.edges)):
           ...
           self.dacc[j][t+self.G.edges[i,j]['time'][self.time]] += self.paxFlow[i,j][t+self.G.edges[i,j]['time'][self.time]]
           self.dacc_spatial[j_region][t+self.G.edges[i, j]['time'][self.time]] += self.paxFlow[i, j][t+self.G.edges[i, j]['time'][self.time]]
    ...
    def reb_step(self, rebAction):
        ...
        for k in range(len(self.edges)):
            ...
            self.dacc[j][t+self.G.edges[i,j]['time'][self.time]] += self.rebFlow[i,j][t+self.G.edges[i,j]['time'][self.time]]
            ...
            elif i[1] <= j[1] and self.rebAction[k] > 0 and i[0]!=j[0]:
...
class Scenario:
    def __init__(self, EV=True, spatial_nodes=4, charging_stations=None, cars_per_station_capacity=None, number_charge_levels=10, charge_levels_per_charge_step=1, energy_distance=None, tf=60, sd=None, tripAttr=None,
        ...
        if EV == True: 
            ...
            self.cars_charging_per_station = np.zeros_like(self.cars_per_station_capacity)
            ...
            for item in total_acc:
                ...
                for region in self.G_spatial.nodes:
                    ...
                    for c in range(self.number_charge_levels):
                        cut_off_charge = int(0.2*self.number_charge_levels)
                        print("cutoff charge init", cut_off_charge)
                        # cut_off_charge = number_charge_levels - 1
    ...
    def add_charge_edges(self):
        for l in range(self.spatial_nodes):
            ...
            for c1 in range(self.number_charge_levels - 1):
                for c2 in range(c1+1, self.number_charge_levels):
