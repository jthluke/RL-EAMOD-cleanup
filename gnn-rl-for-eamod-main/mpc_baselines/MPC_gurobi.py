...
def solve_mpc(env, gurobi_env=None, mpc_horizon=30, return_initial_state=False, noisy=False):
    time = env.time
    ...
    for t in range(mpc_horizon):
        for o in env.region:
            for d in env.region:
                ...
                demand = env.demand[o, d][t + time]
                if noisy:
                    demand = np.random.normal(env.demand[o, d][t + time], env.demand[o, d][t + time] * 0.1 * (t + 5), 1)[0]
