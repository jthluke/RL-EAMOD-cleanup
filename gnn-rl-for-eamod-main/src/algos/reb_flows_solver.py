...
class RebalFlowSolver:  
    def __init__(self, env, desiredAcc, gurobi_env):
        ...
        self.slack_variables = self.m.addMVar(shape=(len(env.nodes)), lb=-100000000, ub=gp.GRB.INFINITY, vtype=gp.GRB.CONTINUOUS, name="slack")
        ...
        self.obj2 = 0
        for n_idx in range(len(env.nodes)):
            self.obj1 += abs(self.slack_variables[n_idx]) * 1e10
