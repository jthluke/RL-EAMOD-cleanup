...
class RebalFlowSolver:  
    def __init__(self, env, desiredAcc, gurobi_env):
        ...
        for n_idx in range(len(env.nodes)):
            ...
            self.cons_charge_graph2[n_idx] = self.m.addConstr(sum(self.flow[incoming_edges]) - sum(self.flow[outgoing_edges]) + self.slack_variables[n_idx] == desiredAcc[n] - env.acc[n][t] ) 
