...
class GNNParser():
    ...
    def parse_obs(self):
        ...
        e = (torch.tensor([self.env.edges[self.env.edges[self.env.edges.index((e[0], e[1]))][0], self.env.edges[self.env.edges.index((e[0], e[1]))][1]]['time'] for e in edges]).view(1, edge_index.shape[1]).float()).squeeze(0).view(self.input_size, len(edges)).T
